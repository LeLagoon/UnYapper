# UnYapper

An extension that summarizes text

## Description
Ever faced an enormous and scary text that you really didn't want to read? This is a chrome extension that's used to __summarize all the yap__ and provide it in a __concise manner__ for you to understand things simply. 

![alt text](<final extension.png>)

## Instructions
1. Enable developer mode on chrome://extensions
2. Load the extension
3. Enable it and pin it
4. Click on the icon of the extension and put in your API Key of ai.hackclub.com
5. Open the webpage of whose text you want to summarize
6. Select the text and right-click it
7. Click on "Select Text"
8. Open the popup and wait for a few seconds
9. And boom! You're done. Read away all you want

If you have any doubts, check out the [demo video](https://cdn.hackclub.com/019e9ec6-25e9-7fcb-b337-4e167826eb33/2026-06-07_02-34-54.mp4)

## Use of AI
Used google's AI overview for debugging at some points